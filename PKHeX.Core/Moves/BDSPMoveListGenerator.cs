    ...
    public static class BDSPMoveListGenerator
    {
        public static void GenerateBDSPMovesCSV(string outputPath, string errorLogPath)
        {
            try
            {
                ...
                {
                    // Check if the species is present in BDSP
                    ...
                    for (byte form = 0; form < forms.Length; form++)
                    {
                        ...
                        // Process egg moves
                        foreach (var moveId in learnSource8BDSP.GetEggMoves(speciesIndex, form))
                        {
                        ...
                        for (int i = 0; i < tmMoves.Length; i++)
                        {
                            if (personalInfo.GetIsLearnTM(i))
                            {
                                ...
                                allMoves[moveId] = Math.Min(allMoves.ContainsKey(moveId) ? allMoves[moveId] : int.MaxValue, 1); // Change 0 to 1 for TM moves
                            }
                        }
